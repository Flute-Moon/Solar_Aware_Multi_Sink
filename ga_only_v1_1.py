...
================================================================================
  ...
  (bs_score) that rewards CH sets able to reach the base station directly
  (PATH A). The combined-script fitness uses 4 terms.

  ----------------------------------------------------------------------------
  VERSION 1.1 CHANGELOG  (Tier-1 defect fixes vs the original ga_only.py)
  ----------------------------------------------------------------------------
  [Fix 1] Orphan MS-CH eliminated. The MS-CH tier now requires >= 2 relay CHs.
          A single lone relay CH no longer gets promoted to a pointless
          MS-CH (empty star + long TX for zero aggregation benefit); it now
          falls back to a direct BS transmission. A safety net also reroutes
          ANY relay CH left unserved by an MS-CH straight to the BS, so no
          aggregated data can ever be silently dropped.
  [Fix 2] Snapshot consistency. The green MS-CH star and the red PATH-C arrow
          are only drawn for MS-CHs that actually have relay followers, so the
          image can no longer contradict its own "MS-USED / MS-SKIPPED" title.
  [Fix 3] bs_score no longer starves coverage. Instead of rewarding "most CHs
          near the BS" (which pulled every CH into the top of the field and
          left the far side uncovered), it now rewards having AT LEAST ONE CH
          able to reach the BS directly, with a much smaller weight. Coverage
          weight was raised to compensate.
  [Fix 4] DIRECT_DIST default changed 0.75*field -> 0.55*field so the MS-CH
          tier actually engages instead of nearly every CH going direct.
  [Fix 5] run_simulation() takes a `seed` argument (default 42) so Monte-Carlo
          / multi-seed averaging is now possible.
  [Fix 6] The previously-computed-but-unused `max_dist` is now used to
          normalise the reworked bs_score.
  ----------------------------------------------------------------------------
...
def get_user_input() -> dict:
    ...
    print("\n  -- Routing Thresholds --")
    # [v1.1 Fix 4] default lowered 0.75 -> 0.55 of the field so the MS-CH tier
    # actually engages instead of almost every CH qualifying as "direct".
...
def get_num_ms(num_relay_chs: int, cfg: dict) -> int:
    ...
    [v1.1 Fix 1] Requires at least TWO relay CHs before an MS-CH is elected.
    An MS-CH only earns its keep when it aggregates traffic from OTHER relay
    CHs; with a single relay CH there is nothing to aggregate, so promoting it
    to an MS-CH just adds overhead and a misleading topology star. In that case
    we return 0 and the lone relay CH transmits directly to the BS instead
    (handled in simulate_round_ga).

        relay = 0 or 1  -> 0   (MS-CH stage skipped)
        relay = 2       -> 1
...
def _evaluate_population(pop: List["Chromosome"],
    ...
    # 5. BS-proximity score --------------------------------------------------
    # [v1.1 Fix 3+6] The original term rewarded the FRACTION of CHs within
    # DIRECT_DIST of the BS. That pulled the whole CH set toward the base
    # station and left the far side of the field uncovered (early deaths).
    #
    # The reworked term rewards only what PATH A actually needs: that AT LEAST
    # ONE CH can reach the BS directly. If none can, it degrades gracefully
    # with the closest CH's normalised distance (using max_dist, which was
    # previously computed but unused -> Fix 6). Its weight is also dropped
    # from 0.15 to 0.10, and coverage raised from 0.30 to 0.35, so the GA no
    # longer trades coverage away for BS-hugging.
    ...
    has_direct = (ch_bs_d <= cfg["DIRECT_DIST"]).any(axis=1)             # (P,)
    closest    = 1.0 - np.minimum(ch_bs_d.min(axis=1) / max_dist, 1.0)   # (P,)
    bs_score   = np.where(has_direct, 1.0, closest).astype(np.float64)
...
def simulate_round_ga(nodes: List[Node],
    ...
    # [v1.1 Fix 1] Safety net: any relay CH that ended up WITHOUT an MS-CH to
    # report to (e.g. the lone-relay case where num_ms == 0, or a leftover
    # from clustering) must still reach the BS. Route it directly instead of
    # letting its aggregated data be silently dropped in Step 9. Such a CH is
    # NOT an MS-CH, so it is added to the direct set and drawn as PATH A.
    for ch in relay_chs:
...
def run_simulation(cfg: dict, seed: Optional[int] = 42):
    # [v1.1 Fix 5] `seed` is now a parameter (default 42 keeps runs
    # reproducible). Pass a different value per run - or None for a
    # non-deterministic run - to average results over many seeds
    # (Monte-Carlo), which the fixed hard-coded seed previously made
    # impossible.
...
def plot_topology_snapshot(nodes: List["Node"],
    ...
    # Everything drawn for the MS tier (green star, red PATH-C arrow, the
    # MS-USED title) is gated on this set, so the picture can never disagree
    # with itself.
    served_ms_ids = {c.assigned_ms for c in relay_chs
                     if c.assigned_ms is not None}
    for n in nodes:
        if not n.alive:
            dead_x.append(n.x)
            dead_y.append(n.y)
    ...
    # Count only relay CHs that truly report to an MS-CH (excludes any lone
    # relay that was rerouted to a direct BS transmission by Fix 1).
    n_relay  = sum(1 for c in relay_chs if c.assigned_ms is not None)
    ...
    for m in ms_chs:
        if m.alive and m.id in served_ms_ids:   # [v1.1 Fix 2] real MS only
    ...
    for c in relay_chs:
        # [v1.1 Fix 1/2] skip MS-CHs and any lone relay rerouted to direct BS
        if c.alive and c.id not in ms_id_set and not c.goes_direct:
